...
# This code is referred from with slight changes: https://gist.github.com/bkozora/d4f1cf0e5cf26acdd377
# 
# This script will search for all instances having a tag with "Backup"
# on it. As soon as it has the instances list, it loops through each instance
# and reference the AMIs of that instance. It then checks whether the latest daily backup
...
#Specify the region in which EC2 Instances are located. Ex: N. Virginia region (us-east-1)
ec = boto3.client('ec2', 'us-east-1')
ec2 = boto3.resource('ec2', 'us-east-1')
images = ec2.images.filter(Owners=["self"])# You can specify your AWS account owner id as well in place of "self" at all the places in this script
...
def lambda_handler(event, context):
    ...
    print "Found %d instances that need evaluation" % len(instances)
    ...
    if backupSuccess == True:
        ...
        for image in imagesList:
            #print image
            desc_image_snapshots = ec.describe_images(ImageIds=[image],Owners=['self',])['Images'][0]['BlockDeviceMappings']
           # print (desc_image_snapshots)
            try:
                for desc_image_snapshot in desc_image_snapshots:
                    snapshot = ec.describe_snapshots(SnapshotIds=[desc_image_snapshot['Ebs']['SnapshotId'],], OwnerIds=['self'])['Snapshots'][0]
